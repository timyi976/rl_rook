...
def eval_many_episodes(env, model, num_episodes=100):
    ...
    for _ in range(num_episodes):
        traces, actions, rewards = infer_one_epsidoe(env, model)
...
if __name__ == "__main__":
    ...
    model_path = "model/cnn_add_merge/44"
    env = gym.make('Rook-v0', map_file=args.map_file, reward_dict={
          ...
          "stay_penalty": 0,               # -3,Penalty for failing to move (hitting a wall or staying in place).
          ...
          "reached_target": 0,             # 10,Reward for reaching the correct target.
          "leave_target_penalty": 0,      # -20,Penalty for leaving a marker's target after reaching it.
          "distance_bonus": 0,              # 1,Scaled reward for reducing the Manhattan distance to the target.
        })
