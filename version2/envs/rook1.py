...
class RookEnv(gym.Env):
    def __init__(self, map_file: str, reward_dict: dict, max_steps: int = 1000, seed: int = None):
    ...
    def _calculate_reward(self, old_map: Map, new_map: Map, marker: int, direction: int) -> float:
        ...
        # for i in range(old_map.n_markers):
        #     if i != marker:
        #         marker_center_coor += np.argwhere(old_map.marker_pos[i] == 1)[0]
        # marker_center_coor /= (old_map.n_markers - 1)
        ...
        if merge_num_delta > 0:
            reward += self.reward_dict["merge_reward"]
