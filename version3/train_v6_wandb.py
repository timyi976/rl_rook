...
from stable_baselines3 import A2C, DQN, PPO, SAC, A2C
from sb3_contrib import RecurrentPPO,ARS,TRPO
...
def arg_parse():
    ...
    parser.add_argument("--run_id", type=str, default="ablation_no_traj")
    ...
    parser.add_argument("--learning_rate", type=float, default=1.05e-4) #1.05e-4
    ...
    parser.add_argument("--val_map_file_dir", type=str, default="./tasks_same_t_test")
    parser.add_argument("--map_file", type=str, default="./tasks/custom_v2.txt")
    parser.add_argument("--n_envs", type=int, default=4)
    parser.add_argument("--policy_net", type=str, default="MlpPolicy")
...
def train(model, epoch_num, timesteps_per_epoch, eval_episode_num, seen_eval_env, unseen_eval_env, save_path, gif_strategy="every"):
    ...
    # infer_and_visualize(seen_eval_env, model, save_path+f"_chess_animation.gif")
    ...
    for epoch in tqdm(range(epoch_num)):
        ...
        seen_avg_score,seen_wire_length = eval_many_episodes(seen_eval_env, model, 6) #eval_episode_num
        unseen_avg_score,unseen_wire_length = eval_many_episodes(unseen_eval_env, model, 1) #eval_episode_num

        # Log metrics to WandB
        wandb.log({
            "epoch": epoch,
            "seen_avg_score": seen_avg_score,
            "unseen_avg_score": unseen_avg_score,
            "seen_wire_length": seen_wire_length,
            "unseen_wire_length": unseen_wire_length
        })

        if gif_strategy == "every":
            infer_and_visualize(seen_eval_env, model, f"chess_animation.gif")

        if seen_avg_score > current_best_reward:
            current_best_reward = seen_avg_score
            best_ckpts.append(epoch)
            if gif_strategy == "best" and seen_avg_score > 10:
                infer_and_visualize(seen_eval_env, model, save_path+f"_chess_animation_{epoch}.gif")
                ...
                # infer_and_visualize(seen_eval_env, model, save_path+"chess_animation6.gif")
            ...
            print(f"Epoch {epoch}: New best seen reward/wire_length: {current_best_reward}/{seen_wire_length}; Current unseen reward/wire_length: {unseen_avg_score}/{unseen_wire_length}")
            with open(os.path.join(save_path,"record.txt"), "a") as f:
                f.write(f"Epoch {epoch}: New best seen reward/wire_length: {current_best_reward}/{seen_wire_length}; Current unseen reward/wire_length: {unseen_avg_score}/{unseen_wire_length}\n")
...
class CustomCNN(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256, kernel_size: int = 3):
        ...
        self.linear = nn.Sequential(
                        ...
                        nn.ReLU(),
                        nn.Linear(256, 128),  # Add one more layer
                        nn.ReLU()
                    )
...
def main():
    ...
    wandb.init(
        project="RookEnv-Ablation-Exp_mlp_500epoch",
        name=args.run_id,
        config={
    ...
    policy_kwargs = dict(
        ...
        features_extractor_kwargs=dict(features_dim=128, kernel_size=3),  # Customize kernel size
    ...
    # model = algorithm.load("./models/Same_t_again/786")
