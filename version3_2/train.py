...
def arg_parse():
    ...
    parser.add_argument("--run_id", type=str, default="custom2_v2_debug_no_one_marker")
    ...
    parser.add_argument("--env", type=str, default="RookEnv_v4")
...
class CustomCNN(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256, kernel_size: int = 3):
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(n_input_channels, 64, kernel_size=kernel_size, stride=1, padding=kernel_size // 2),
            nn.ReLU(),
            nn.Conv2d(64, 128, kernel_size=kernel_size, stride=1, padding=kernel_size // 2),
            nn.ReLU(),
            nn.Conv2d(128, 128, kernel_size=kernel_size, stride=2, padding=kernel_size // 2),
            nn.ReLU(),
            nn.Conv2d(128, 128, kernel_size=kernel_size, stride=1, padding=kernel_size // 2),
            nn.ReLU(),
            nn.Flatten()
        )
...
def main():
    ...
    def make_env():
        ...
        env = gym.make('Rook-v0', map_file=args.map_file, reward_dict = {
        ...
        # env = gym.make('Rook-v0', map_file=args.map_file, reward_dict = {
        #     "step_penalty": -2,               # -1,Penalty for every step to encourage efficiency.
        #     "blockage_penalty": 0,           # -3,Penalty for moving into a blocked cell.
        #     "stay_penalty": -0.2,               # -3,Penalty for failing to move (hitting a wall or staying in place).
        #     "source_collision_penalty": -2,   # -3,Penalty for moving into another marker's initial position.
        #     "target_collision_penalty": -2,   # -3,Penalty for moving into another marker's target position.
        #     "merge_reward": 1.2,               # 20,Reward for merging markers or transitioning into special states.
        #     "reached_target": 20,             # 10,Reward for reaching the correct target.
        #     "leave_target_penalty": -40,      # -20,Penalty for leaving a marker's target after reaching it.
        #     "target_distance_bonus": 0.4,              # 1,Scaled reward for reducing the Manhattan distance to the target.
        #     "merge_bonus": 0.4,
        # }
        # )
        return env
    ...
    if not os.path.exists("model"):
        os.makedirs("model")
    train(model, args.epoch_num, args.timesteps_per_epoch, args.eval_episode_num, eval_env, f"model/{args.run_id}", args.gif_strategy)
