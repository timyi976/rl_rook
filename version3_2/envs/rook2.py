...
class Map():
    def __init__(self, n_markers, map_dim, prev_marker_pos, marker_pos, initial_pos, terminal_pos, block_pos):
        self.n_markers = n_markers
        self.map_dim = map_dim
        self.marker_pos = marker_pos
        self.prev_marker_pos = prev_marker_pos
...
class RookEnv(gym.Env):
    def __init__(self, map_file: str, reward_dict: dict, max_steps: int = 100, seed: int = None):
        ...
        self.observation_space = spaces.Box(low=0, high=1, shape=(self.n_markers+3, self.map_dim[0], self.map_dim[1]), dtype=np.int32)
    ...
    def _construct_state(self, map: Map) -> np.ndarray:
        ...
        """
        # Layer 1: Marker positions (n_markers layers)
        markers_layer = map.marker_pos

        # Layer 2: Blockages (1 layer)
        blockages_layer = map.block_pos[np.newaxis, :, :]  # Expand to (1, height, width)

        # Layer 3: Initial positions (1 layer)
        initial_positions_layer = (map.initial_pos >= 0).astype(np.int32)[np.newaxis, :, :]  # Binary layer for initial positions

        # Layer 4: Targets (1 layer)
        targets_layer = (map.terminal_pos >= 0).astype(np.int32)[np.newaxis, :, :]  # Binary layer for targets

        # Combine all layers
        state = np.concatenate([markers_layer, blockages_layer, initial_positions_layer, targets_layer], axis=0)
        return state
    ...
    def _move_marker(self, map: Map, marker: int, direction: int) -> Map:
        ...
        if valid:
            prev_marker_pos = map.marker_pos.copy()
        ...
        return Map(
            ...
            map_dim=map.map_dim,
            prev_marker_pos=prev_marker_pos,
            marker_pos=marker_pos,
    ...
    def _calculate_reward(self, map: Map, marker) -> float:
        ...
        """
        reward = 0
        prev_marker_coor = np.argwhere(map.prev_marker_pos[marker] == 1)[0] 
        cur_marker_coor = np.argwhere(map.marker_pos[marker] == 1)[0]
        direction = cur_marker_coor - prev_marker_coor
        terminal_coor = np.argwhere(map.terminal_pos == marker)[0]
        ...
        # Add stay penalty
        if np.all(map.prev_marker_pos == map.marker_pos):
        ...
        prev_merge_num = np.max(np.sum(map.prev_marker_pos, axis=0))  # Shape: (height, width)
        cur_merge_num = np.max(np.sum(map.marker_pos, axis=0))  # Shape: (height, width)
        merge_num_delta = cur_merge_num - prev_merge_num
        if merge_num_delta > 0 and np.dot(direction, terminal_coor - prev_marker_coor) > 0:
            reward += self.reward_dict["distance_bonus"] * (map.n_markers) * prev_merge_num
    ...
    def step(self, action: tuple):
        ...
        self.map.prev_marker_pos = self.map.marker_pos
        ...
        reward = self._calculate_reward(self.map,marker)
